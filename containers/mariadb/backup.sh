...
export CMD_MYSQL="mysql -h 127.0.0.1 -P ${MYSQL_PORT} -u${MYSQL_ROOT_USER} -p${MYSQL_PASSWORD}"
export CMD_MYSQLDUMP="mysqldump --host 127.0.0.1 --port ${MYSQL_PORT} -u${MYSQL_ROOT_USER} -p${MYSQL_PASSWORD} --events --ignore-table=mysql.event --default-character-set=${DB_CHARACTER_SET} --skip-comments"
